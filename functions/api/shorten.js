export async function onRequestPost(context) {
  ...
  try {
    ...
    if (finalPath) {
      ...
      // check existence
      const existing = await env.DB.prepare('SELECT id FROM links WHERE path = ?').bind(finalPath).first();
      if (existing) {
      ...
      do {
        ...
        const collision = await env.DB.prepare('SELECT id FROM links WHERE path = ?').bind(finalPath).first();
