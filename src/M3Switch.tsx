...
export default function Switch({
  ...
  // Handle is 16px base, scales to 24px (1.5x) or 28px (1.75x).
  // Scaling happens from center, so we offset x to keep gaps even.
  
  const hasUncheckedIcon = icons === "both";
  const restingScale = (checked || hasUncheckedIcon) ? 1.5 : 1;
  ...
  const restingX = checked ? 26 : 10;

  const springX = useSpring(restingX, { stiffness: 450, damping: 30, mass: 0.8 });
  const springScale = useSpring(restingScale, { stiffness: 450, damping: 30, mass: 0.8 });
